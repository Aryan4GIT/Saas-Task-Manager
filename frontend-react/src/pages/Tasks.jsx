...
import { Plus, Edit2, Trash2, Calendar, Filter, Check, CheckCircle, XCircle, Shield, Clock } from 'lucide-react';
...
export default function Tasks() {
  ...
  const getWorkflowActions = (task) => {
    ...
    if (task.status === 'todo' || task.status === 'in_progress') {
      actions.push({ label: 'Mark Done', onClick: () => handleMarkDone(task.id), className: 'bg-gradient-to-r from-teal-500 to-green-500 text-white shadow-md shadow-teal-500/50 hover:shadow-lg hover:shadow-teal-500/60 transform hover:-translate-y-0.5 transition', icon: <Check className="w-4 h-4" /> });
    }
